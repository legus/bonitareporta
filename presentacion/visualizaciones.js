...
//Si el fetch falla o el servidor no está listo, 
// el dashboard cae en los datos dummy automáticamente como respaldo


// Datos dummy respaldo si el API no está disponible
...
// Funcion Principal pinta el dashboard con los datos

function pintarDashboard(K1, K2, K4, K5) {
...
// FETCH — intenta obtener datos reales del API dummy
// Si falla, usa los datos dummy locales como respaldo

fetch('http://localhost:[puerto]/bonitareporta/logica/api/dummy_incidencias.php')
  .then(res => res.json())
  .then(res => {
    if (res.status === 'success') {
      // Mapea la respuesta del API al formato esperado por el dashboard
      const data = res.data.incidencias;

      const K1 = ['Hueco','Luz dañada','Basura','Otro'].map(tipo => ({
        tipo,
        a: data.filter(d => d.tipo === tipo && d.estado === 'activo').length,
        p: data.filter(d => d.tipo === tipo && d.estado === 'en_proceso').length,
      }));

      const K2 = data.reduce((acc, d) => {
        if (!acc.find(x => x.tipo === d.tipo)) {
          acc.push({tipo: d.tipo, h: d.tiempo_hrs, e: d.tiempo_hrs > 7 ? 'c' : d.tiempo_hrs > 5 ? 'w' : 'ok'});
        }
        return acc;
      }, []);

      const K5 = ['Hueco','Basura','Luz dañada','Otro'].map(t => ({
        t, c: data.filter(d => d.tipo === t).length
      }));

      pintarDashboard(K1, K2, DUMMY_K4, K5);
    } else {
      pintarDashboard(DUMMY_K1, DUMMY_K2, DUMMY_K4, DUMMY_K5);
    }
  })
  .catch(() => {
    // El API no está disponible, se usan los datos dummy locales
    pintarDashboard(DUMMY_K1, DUMMY_K2, DUMMY_K4, DUMMY_K5);
  });